...
const projects = [
  ...
  { id: "framework", no: "03", name: "Градоэкологический каркас", type: "СИСТЕМА", essence: "Единая пространственная система лесов, рек, парков, улиц и новых районов.", problem: "Природные территории и проекты разобщены.", output: "Общегородская рельефно-гидрологическая схема.", next: "Цифровая модель, карта конфликтов и приоритетов." },
  { id: "rivers", no: "04", name: "16 рек — одна система", type: "ВОДА", essence: "Превратить городские водотоки из отдельных объектов в управляемый водно-зелёный каркас.", problem: "Водотоки рассматриваются фрагментарно.", output: "Паспорта 16 водосборов и 5–7 проектов первой очереди.", next: "Рогатка, Еланька, Сусуя, Красносельская, Уюновка, Придорожный." },
  { id: "boulevards", no: "05", name: "Экобульвары", type: "УЛИЦЫ", essence: "Улицы, которые дают тень, задерживают воду и связывают город пешими маршрутами.", problem: "Улицы ускоряют сток и перегреваются.", output: "Тень, биоканавы, дождевые сады и связные маршруты.", next: "Мира, Сахалинская, Горького, Украинская, Ленина." },
  { id: "north", no: "06", name: "Северная долина", type: "РАЙОН", essence: "Зелёный район нового типа: развитие без усиления стока и разрыва природных связей.", problem: "Новая застройка может усилить сток и разорвать зелёные связи.", output: "Водно-зелёный контур и требования к девелоперам.", next: "Мастер-схема и дорожная карта 2026–2029." },
  { id: "renewal", no: "07", name: "Экореновация промтерриторий", type: "КРТ", essence: "Переустройство промзон через управление стоком, зелёные буферы и новые связи.", problem: "Твёрдые покрытия усиливают загрязнённый сток.", output: "Водные паспорта, зелёные буферы и биоретенция.", next: "Пилот: проспект Мира — улица Украинская." },
];
...
function Cover({go}:{go:(n:number)=>void}){return <div className="cover-layout"><div className="cover-copy"><h1>От данных<br/>о здоровье —<br/><em>к проектам города</em></h1><p>Результаты апробации Стандарта здоровьеформирующей среды</p><div className="cover-actions"><button className="primary" onClick={()=>go(1)}>Начать показ <span>→</span></button><button onClick={()=>go(4)}>Сразу к флагману</button></div></div><div className="cover-orbit"><div className="orbit one"><span className="orbit-label">СРЕДА</span></div><div className="orbit two"><span className="orbit-label">ЗДОРОВЬЕ</span></div><div className="orbit three"><span className="orbit-label">ПРОЕКТЫ</span>{["Зелёный хребет","Детская безопасность","Градоэкологический каркас","16 рек","Экобульвары","Северная долина","Экореновация"].map((name,i)=><i className={`project-node p${i+1}`} data-project={name} key={name}><small>{i+1}</small></i>)}</div><div className="orbit-core"><b>7</b><span>проектных решений</span><small>для Южно-Сахалинска</small></div></div></div>}
...
function ProjectPortfolio({selected,setSelected,go}:{selected:typeof projects[number];setSelected:(x:string)=>void;go:(n:number)=>void}){
  return <div className="projects-layout">
    <div className="projects-head"><div><div className="eyebrow">02 · ПРЕДЛАГАЕМЫЕ ПРОЕКТЫ</div><h2>Семь проектов.<br/><em>Одна система изменений.</em></h2></div><p>Наведите на проект, чтобы увидеть задачу, результат и первый шаг.</p></div>
